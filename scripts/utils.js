...
const updateDB = () => {
    if (!database.length) {
        downloadDB(databaseUrl).then((_database) => {
            database = _database;
            browser.webRequest.onCompleted.addListener(scan, filter, []);
...
const setBadgeCount = (len) => {
    browser.browserAction.setBadgeText({ "text": '' + len });
    browser.browserAction.setBadgeBackgroundColor({ color: len > 0 ? [255, 0, 0, 255] : [0, 0, 255, 0] });
